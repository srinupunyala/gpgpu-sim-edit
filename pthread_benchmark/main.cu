...
cudaEvent_t app_local_event[TOTALAPPS];
cudaEvent_t app_global_event[TOTALAPPS];
...
void *app_1(void *app_arg) 
{
    ...
    loop_back_1:
	if ((!modeflag && (cudaEventQuery(app1_local_event) == cudaSuccess) && (cudaEventQuery(app2_global_event) != cudaSuccess)) || (modeflag && (cudaEventQuery(app1_global_event) == cudaSuccess) && ((cudaEventQuery(app2_global_event) != cudaSuccess) || (cudaEventQuery(app3_global_event) != cudaSuccess)))) {
...
void *app_2(void *app_arg) 
{
    ...
    loop_back_2:
	if ((!modeflag && (cudaEventQuery(app1_local_event) != cudaSuccess) && (cudaEventQuery(app2_global_event) == cudaSuccess)) || (modeflag && ((cudaEventQuery(app1_local_event) != cudaSuccess) || 
(cudaEventQuery(app3_local_event) != cudaSuccess)) && (cudaEventQuery(app2_global_event) == cudaSuccess))) {
...
void *app_3(void *app_arg) 
{
    ...
    loop_back_3:
	if (((cudaEventQuery(app1_local_event) != cudaSuccess) || (cudaEventQuery(app2_local_event) != cudaSuccess) ) && ((cudaEventQuery(app3_global_event)== cudaSuccess) || (cudaEventQuery(app3_local_event) == cudaSuccess))) {
...
void *all_apps(void *app_arg){
    ...
    char *name = my_data->app_name;
    start_apps:
    cutilSafeCall(cudaEventCreate(&app_local_event[mystreamid]));
    	
	callApp(name,stream[mystreamid], &mutexapp, appflag);


    cutilSafeCall(cudaEventRecord(app_local_event[mystreamid], stream[mystreamid]));
    cutilSafeCall(cudaEventRecord(app_local_event[mystreamid], stream[mystreamid]));
    
    loop_back_all:
    int check_all_status=0;
	
    if ((cudaEventQuery(app_global_event[mystreamid])== cudaSuccess)){
		
		for(int i=0;i<TOTALAPPS;i++)
		{
			if(i!=mystreamid){
				if ((cudaEventQuery(app_global_event[i])!=cudaSuccess)){
					check_all_status++;
				}
			}
		}
		if (check_all_status>0) {
			...
			relaunch_count_all[mystreamid]++;
			cudaEventDestroy(app_local_event[mystreamid]);
			goto start_apps;
		}
    }
